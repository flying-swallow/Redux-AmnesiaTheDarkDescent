...
// Linear offset along the surface normal. The previous ULP-bump variant
// scaled `intScale` by depth (1024*(1+20000*depth)), which at depth~0.99
// displaces by tens-to-thousands of world units depending on |p| — the
// per-pixel variance produced shadow-ray speckle. Matches OffsetRayBindless.
vec3 OffsetRayGbuffer(in vec3 p, in vec3 n, in float depth)
{
    const float kSelfIntersectEps = 0.001;
    return p + n * kSelfIntersectEps;
}
