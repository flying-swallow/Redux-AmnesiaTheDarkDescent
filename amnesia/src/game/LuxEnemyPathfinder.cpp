...
void cLuxEnemyPathfinder::OnRenderSolid(DebugDraw* apDebugDraw)
{
	...
	for(int i=0; i<mpNodeContainer->GetNodeNum(); ++i)
	{
		...
		apDebugDraw->DebugDrawSphere(pNode->GetPosition(), 0.3f,cColor(0.4f,1));

		for(int j=0; j < pNode->GetEdgeNum(); ++j)
		{
			cAINodeEdge *pEdge = pNode->GetEdge(j);

			apDebugDraw->DebugDrawLine(pNode->GetPosition(),pEdge->mpNode->GetPosition(),cColor(0.4f,0.4f,0.4f,1));
		}
	}
	...
	apDebugDraw->DebugDrawSphere(vGoalPos,0.2f, cColor(1,0,1));
	...
	for(; it != mlstPathNodes.end(); ++it)
	{
		...
		apDebugDraw->DebugDrawSphere(vNodePos,0.2f, cColor(1,0,1));
		apDebugDraw->DebugDrawLine(vLastVec, vNodePos,cColor(1,0,1));
	...
	//}
	apDebugDraw->DebugDrawSphere(vStartPos,0.2f,cColor(1,0,1));
	apDebugDraw->DebugDrawLine(vLastVec, vStartPos,cColor(1,0,1));
