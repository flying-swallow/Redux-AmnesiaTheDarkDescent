-- Vendored from https://github.com/tarruda/premake-export-compile-commands
-- (master @ f32b89638258b130ee99719e587e310c9a4255e2), with one local fix:
-- getCommonFlags now picks getcflags vs getcxxflags per file instead of
-- always using getcflags (upstream applied the C dialect's -std= flag to
-- .cpp files too). Adds the `export-compile-commands` premake action, which
-- writes compile_commands/<config>.json under the workspace location by
-- reading the already-resolved project model directly -- no build required,
-- and every path (both `directory` and each `-I`) comes out absolute.
...
function m.getCommonFlags(cfg, node)
  ...
  -- getcflags/getcxxflags carry the -std= dialect (cdialect vs cppdialect);
  -- picking the wrong one for a given file makes clangd reject the command
  -- (e.g. "-std=gnu11 not allowed with C++" on a .cpp file).
...
function m.getFileFlags(prj, cfg, node)
  return table.join(m.getCommonFlags(cfg, node), {
