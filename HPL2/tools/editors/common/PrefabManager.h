...
// Prefab lifecycle carried by cPrefabManager's broadcast (see mOnPrefabChanged).
enum ePrefabEvent
{
	...
	ePrefabEvent_Removed,	// the pending definition was dropped (disk is truth again)
};
...
public:
	explicit cPrefabResource(const tString& asKey)
		: iResourceBase(asKey, cString::To16Char(asKey), 0), mpDoc(NULL), mbDirty(false) {}
	~cPrefabResource();
	...
	tinyxml2::XMLDocument*	mpDoc;		// owned in-memory definition (NULL = on-disk only)
...
//	is the sole subscriber: it scans its own entities by filename and reloads
//	the matches (rebuilding through the pending-first entity load path, which
//	resolves to the pending doc if present, else disk — so Added/Modified/
...
public:
	...
	Event<const tString&, ePrefabEvent>& OnPrefabChanged() { return mOnPrefabChanged; }
	...
	void SetPendingDoc(const tString& asEntFile, tinyxml2::XMLDocument* apDoc, bool abBroadcast = true);
