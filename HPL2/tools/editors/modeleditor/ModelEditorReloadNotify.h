...
#ifndef MODELEDITOR_RELOAD_NOTIFY_H
#define MODELEDITOR_RELOAD_NOTIFY_H

#include <string>

//----------------------------------------------------------------------
// Fire-and-forget: POST a JSON-RPC tools/call for reload_entity_file(path) to
// the LevelEditor's MCP server at host:port, so it re-reads the just-saved .ent
// and updates its placed instances live. The socket I/O runs on a detached
// thread, so a save never blocks on it. No-op if alPort<=0 or asEntFile empty.
//
...
#endif // MODELEDITOR_RELOAD_NOTIFY_H
