...
{
	if(alPort<=0 || asPsFile.empty())
		return;

	const std::string sHost = asHost.empty() ? std::string("127.0.0.1") : asHost;

	//////////////////////////////////////////////////////////////////////
	// Build the JSON-RPC 2.0 tools/call envelope for reload_particle_system.
	...
	w.Key("params");
	w.StartObject();
	w.Key("name");      w.String("reload_particle_system");
	w.Key("arguments");
	w.StartObject();
	w.Key("path");    w.String(asPsFile.c_str(), (rapidjson::SizeType)asPsFile.size());
	w.EndObject();
	w.EndObject();
	w.EndObject();
