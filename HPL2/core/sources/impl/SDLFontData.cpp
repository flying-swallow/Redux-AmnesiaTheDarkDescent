...
bool cSDLFontData::CreateFromBitmapFile(const tWString &asFileName) {
  ...
  for (; pPageElem != NULL;
       pPageElem = pPageElem->NextSiblingElement("page"), ++lCount) {
    ...
    cBitmap *pBitmap =
        mpResources->GetBitmapLoaderHandler()->LoadBitmap(sFilePath, 0);
    if (pBitmap == NULL) {
    ...
    if (pBitmap->GetPixelFormat() == ePixelFormat_Luminance) {
      ...
      pTempBitmap->CreateData(pBitmap->GetSize(), ePixelFormat_LuminanceAlpha,
                              0, 0);
    ...
    // Log("Loaded bitmap %s, bpp: %d pixelformat:
    // %d\n",sFileName.c_str(),pBitmap->GetBytesPerPixel(),
    // pBitmap->GetPixelFormat());

    ///////////////////////
    // Create a texture from bitmap (do not want to load it from texture manager
    // since that would delete the texture on its own).
  ...
  for (; pCharElem != NULL; pCharElem = pCharElem->NextSiblingElement("char")) {
    ...
    if (lId < 0 || lId >= (int)mvGlyphs.size()) {
      Warning("Font '%s' contain glyph with invalid id: %d. Skipping loading "
              "of it!\n",
    ...
    SharedResourceHandle<cFrameSubImage> pImage =
        pFrameTexture->CreateCustomImage(cVector2l(lX, lY), cVector2l(lW, lH));
...
bool cSDLFontData::CreateFromFontFile(const tWString &asFileName, int alSize,
  ...
  if(pFont==NULL){
          Error("Error when opening '%s':
%s\n",asFileName.c_str(),TTF_GetError()); return false;
  }
...
/*cGlyph* cSDLFontData::RenderGlyph(TTF_Font* apFont,unsigned short aChar, int
alFontSize)
{
        ...
        TTF_GlyphMetrics(apFont, aChar, &vMin.x, &vMax.x, &vMin.y, &vMax.y,
&lAdvance);
        ...
        SDL_Surface* pTempSurface = SDL_CreateRGBSurface(	SDL_SWSURFACE,
vSize.x, vSize.y,32, 0x000000ff,0x0000ff00,0x00ff0000,0xff000000);
...
//Set the alpha of the bitmap to the average color.
        ...
        for(int y=0;y<vSize.y;y++)
                ...
                {
                        unsigned char* Pix =
&PixBuffer[y*pBmp->GetWidth()*lBmpSize + x*lBmpSize];
...
mpResources->GetImageManager()->CreateFromBitmap("",pBmp); if(pImage==NULL){
                FatalError("Couldn't create image for bitmap\n");
                return NULL;
        }
        // Adopt the count-1 image from CreateFromBitmap into a handle for the
glyph. SharedResourceHandle<cFrameSubImage>
hImage(mpResources->GetImageManager(), pImage);

        ////////////////////////////////////
        //Create the Glyph
        int lHeight = TTF_FontHeight(apFont);
        cVector2l vOffset = cVector2l(vMin.x, alFontSize - vMax.y);//(lHeight -
vSize.y) - vMin.y);

        cGlyph* pGlyph = CreateGlyph(hImage,vOffset,vSize,alFontSize,lAdvance);
