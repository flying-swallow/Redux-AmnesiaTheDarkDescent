...
cLowLevelGraphicsSDL::cLowLevelGraphicsSDL() {
  ...
  mpVertexArray =
      (float *)hplMalloc(sizeof(float) * mlBatchStride * mlBatchArraySize);
  mpIndexArray = (unsigned int *)hplMalloc(
      sizeof(unsigned int) * mlBatchArraySize); // Index is one int.

  for (int i = 0; i < kMaxTextureUnits; i++) {
    mpTexCoordArray[i] =
        (float *)hplMalloc(sizeof(float) * 3 * mlBatchArraySize);
...
cLowLevelGraphicsSDL::~cLowLevelGraphicsSDL() {
  ...
  if (mbInitHasBeenRun) {
    ...
    SDL_SetGammaRamp(mvStartGammaArray[0], mvStartGammaArray[1],
                     mvStartGammaArray[2]);
#endif
  }
...
void CALLBACK OGLDebugOutputCallback(GLenum alSource, GLenum alType,
                                     GLuint alID, GLenum alSeverity,
                                     GLsizei alLength, const GLchar *apMessage,
                                     GLvoid *apUserParam) {
  Log("Source: %d Type: %d Id: %d Severity: %d '%s'\n", alSource, alType, alID,
      alSeverity, apMessage);
}
...
bool cLowLevelGraphicsSDL::Init(int alWidth, int alHeight, int alDisplay,
                                int alBpp, int abFullscreen,
                                int alMultisampling,
...
void cLowLevelGraphicsSDL::SetupGL() {
  ...
  glScissor(mvScissorPos.x,
            (mvScissorSize.y - mvScissorPos.y - 1) - mvScissorSize.y,
            mvScissorSize.x, mvScissorSize.y);
  glDisable(GL_BLEND);
...
int cLowLevelGraphicsSDL::GetCaps(eGraphicCaps aType) {
  ...
  case eGraphicCaps_ShaderModel_2:
    return (GLEW_ARB_fragment_program || GLEW_ARB_fragment_shader)
               ? 1
               : 0; // Mac always support this, so not a good test.
#ifdef __APPLE__
  case eGraphicCaps_ShaderModel_3:
    return 0; // Force return false for OS X as dynamic branching doesn't work
              // well (it's slow)
...
void cLowLevelGraphicsSDL::SetVsyncActive(bool abX, bool abAdaptive) {
  ...
  if (GLX_SGI_swap_control) {
    GLXSWAPINTERVALPROC glXSwapInterval =
        (GLXSWAPINTERVALPROC)glXGetProcAddress((GLubyte *)"glXSwapIntervalSGI");
    glXSwapInterval(abX ? (abAdaptive ? -1 : 1) : 0);
  } else if (GLX_MESA_swap_control) {
    GLXSWAPINTERVALPROC glXSwapInterval =
        (GLXSWAPINTERVALPROC)glXGetProcAddress(
...
iGpuShader *cLowLevelGraphicsSDL::CreateGpuShader(const tString &asName,
                                                  eGpuShaderType aType) {
  ;
...
iTexture *cLowLevelGraphicsSDL::CreateTexture(const tString &asName,
                                              eTextureType aType,
...
iDepthStencilBuffer *cLowLevelGraphicsSDL::CreateDepthStencilBuffer(
    const cVector2l &avSize, int alDepthBits, int alStencilBits) {
  ;
...
void cLowLevelGraphicsSDL::CopyFrameBufferToTexure(
    iTexture *apTex, const cVector2l &avPos, const cVector2l &avSize,
    const cVector2l &avTexOffset) {
  ;
  ...
  // Log(" Copying current to texture Pos: %d:%d Size: %dx%d TextureOffset:
  // %d:%d\n", 	vPos.x, vPos.y, vSize.x, vSize.y, vTexPos.x, vTexPos.y);

  SetTexture(0, apTex);
  glCopyTexSubImage2D(GetGLTextureTargetEnum(apTex->GetType()), 0, vTexPos.x,
                      vTexPos.y, vPos.x, vPos.y, vSize.x, vSize.y);
}

//-----------------------------------------------------------------------

cBitmap *
cLowLevelGraphicsSDL::CopyFrameBufferToBitmap(const cVector2l &avScreenPos,
  ...
  if (mpFrameBuffer) {
    ...
    if (pColorBuffer && pColorBuffer->GetFrameBufferAttachmentType() ==
  ...
  //////////////////////////////
  // Copy pixels
  glReadPixels(vPos.x, vPos.y, vSize.x, vSize.y,
               PixelFormatToGLFormat(pixelFormat), GL_UNSIGNED_BYTE,
...
void cLowLevelGraphicsSDL::SetCurrentFrameBuffer(iFrameBuffer *apFrameBuffer,
  ...
  if (true) // mpFrameBuffer != apFrameBuffer)
  {
    ...
    if (mpFrameBuffer) {
      cFrameBufferGL *pFrameBufferGL =
          static_cast<cFrameBufferGL *>(mpFrameBuffer);
  ...
  ///////////////////////////////////////////////////////
  // Set Viewport
  if (true) // mvFrameBufferPos != vFrameBufferPos || mvFrameBufferSize !=
            // vFrameBufferSize)
  {
    ...
    if (mpFrameBuffer) {
      vFrameBufferPos.y =
          (mpFrameBuffer->GetSize().y - vFrameBufferSize.y) - vFrameBufferPos.y;
    } else {
      vFrameBufferPos.y =
          (mvScreenSize.y - vFrameBufferSize.y) - vFrameBufferPos.y;
    }
    glViewport(vFrameBufferPos.x, vFrameBufferPos.y, vFrameBufferSize.x,
               vFrameBufferSize.y);
  }
}
...
void cLowLevelGraphicsSDL::SetFrameBufferDrawTargets(int *apTargets,
                                                     int alNumOfTargets) {
  ;
...
void cLowLevelGraphicsSDL::SetColorWriteActive(bool abR, bool abG, bool abB,
                                               bool abA) {
  ;

  if (mColorWrite.r == abR && mColorWrite.g == abG && mColorWrite.b == abB &&
      mColorWrite.a == abA) {
    return;
  }
  mColorWrite.r = abR;
...
void cLowLevelGraphicsSDL::SetStencil(eStencilFunc aFunc, int alRef,
                                      unsigned int aMask, eStencilOp aFailOp,
                                      eStencilOp aZFailOp,
                                      eStencilOp aZPassOp) {
  ;
...
void cLowLevelGraphicsSDL::SetStencilTwoSide(
    eStencilFunc aFrontFunc, eStencilFunc aBackFunc, int alRef,
    unsigned int aMask, eStencilOp aFrontFailOp, eStencilOp aFrontZFailOp,
    eStencilOp aFrontZPassOp, eStencilOp aBackFailOp, eStencilOp aBackZFailOp,
  ...
  if (GLEW_EXT_stencil_two_side) {
    ...
    glStencilOp(GetGLStencilOpEnum(aFrontFailOp),
    ...
    glStencilOp(GetGLStencilOpEnum(aBackFailOp),
  ...
  // Ati implementation
    ...
    glStencilFuncSeparateATI(GetGLStencilFuncEnum(aFrontFunc),
                             GetGLStencilFuncEnum(aBackFunc), alRef, aMask);
  } else {
...
void cLowLevelGraphicsSDL::SetScissorRect(const cVector2l &avPos,
                                          const cVector2l &avSize) {
  ;
  ...
  glScissor(avPos.x, (vFrameBufferSize.y - avPos.y) - avSize.y, avSize.x,
            avSize.y);
}
...
void cLowLevelGraphicsSDL::SetBlendFunc(eBlendFunc aSrcFactor,
                                        eBlendFunc aDestFactor) {
  ;
...
void cLowLevelGraphicsSDL::SetBlendFuncSeparate(eBlendFunc aSrcFactorColor,
                                                eBlendFunc aDestFactorColor,
                                                eBlendFunc aSrcFactorAlpha,
                                                eBlendFunc aDestFactorAlpha) {
  ;

  if (GLEW_EXT_blend_func_separate) {
    glBlendFuncSeparateEXT(
        GetGLBlendEnum(aSrcFactorColor), GetGLBlendEnum(aDestFactorColor),
        GetGLBlendEnum(aSrcFactorAlpha), GetGLBlendEnum(aDestFactorAlpha));
  } else {
    glBlendFunc(GetGLBlendEnum(aSrcFactorColor),
...
void cLowLevelGraphicsSDL::SetPolygonOffset(float afBias,
                                            float afSlopeScaleBias) {
  ;
...
void cLowLevelGraphicsSDL::SetOrthoProjection(const cVector2f &avSize,
...
void cLowLevelGraphicsSDL::SetOrthoProjection(const cVector3f &avMin,
...
void cLowLevelGraphicsSDL::SetTextureEnv(eTextureParam aParam, int alVal) {
  ...
  if (aParam == eTextureParam_ColorFunc || aParam == eTextureParam_AlphaFunc) {
    glTexEnvi(GL_TEXTURE_ENV, lParam,
              GetGLTextureFuncEnum((eTextureFunc)alVal));
  } else if (aParam >= eTextureParam_ColorSource0 &&
             aParam <= eTextureParam_AlphaSource2) {
    glTexEnvi(GL_TEXTURE_ENV, lParam,
              GetGLTextureSourceEnum((eTextureSource)alVal));
  } else if (aParam >= eTextureParam_ColorOp0 &&
             aParam <= eTextureParam_AlphaOp2) {
...
void cLowLevelGraphicsSDL::DrawQuadMultiTex(const tVertexVec &avVtx,
  ...
  {
    for (int i = 0; i < 4; i++) {
      glMultiTexCoord3fARB(GL_TEXTURE0_ARB, avVtx[i].tex.x, avVtx[i].tex.y,
                           avVtx[i].tex.z);

      for (int unit = 0; unit < lExtraUnits; ++unit) {
        glMultiTexCoord3fARB(
            GL_TEXTURE0_ARB + unit + 1, avExtraUvs[unit * 4 + i].x,
            avExtraUvs[unit * 4 + i].y, avExtraUvs[unit * 4 + i].z);
      }
...
void cLowLevelGraphicsSDL::DrawBoxMinMax(const cVector3f &avMin,
                                         const cVector3f &avMax, cColor aCol) {
  ;
...
void cLowLevelGraphicsSDL::DrawSphere(const cVector3f &avPos, float afRadius,
                                      cColor aCol) {
  ;
  ...
  glBegin(GL_LINES);
  {
    ...
    // Z Circle:
    for (float a = 0; a < k2Pif; a += afAngleStep) {
      glVertex3f(avPos.x + cos(a) * afRadius, avPos.y + sin(a) * afRadius,
                 avPos.z);

      glVertex3f(avPos.x + cos(a + afAngleStep) * afRadius,
                 avPos.y + sin(a + afAngleStep) * afRadius, avPos.z);
    }
  }
  glEnd();
}
...
void cLowLevelGraphicsSDL::DrawLineQuad(const cRect2f &aRect, float afZ,
                                        cColor aCol) {
  ;
...
void cLowLevelGraphicsSDL::DrawLineQuad(const cVector3f &avPos,
                                        const cVector2f &avSize, cColor aCol) {
  ;
...
void cLowLevelGraphicsSDL::AddVertexToBatch_Size2D(const cVertex *apVtx,
                                                   const cVector3f *avTransform,
                                                   const cColor *apCol,
                                                   const float &mfW,
                                                   const float &mfH) {
...
void cLowLevelGraphicsSDL::AddVertexToBatch_Raw(const cVector3f &avPos,
                                                const cColor &aColor,
...
void cLowLevelGraphicsSDL::AddTexCoordToBatch(unsigned int alUnit,
...
void cLowLevelGraphicsSDL::SetBatchTextureUnitActive(unsigned int alUnit,
                                                     bool abActive) {
  ;
  ...
  if (abActive == false) {
    glTexCoordPointer(3, GL_FLOAT, sizeof(float) * mlBatchStride,
                      &mpVertexArray[7]);
  } else {
...
void cLowLevelGraphicsSDL::FlushTriBatch(tVtxBatchFlag aTypeFlags,
                                         bool abAutoClear) {
  ;
...
void cLowLevelGraphicsSDL::FlushQuadBatch(tVtxBatchFlag aTypeFlags,
                                          bool abAutoClear) {
  ;
...
#ifdef WITH_CG
void cLowLevelGraphicsSDL::InitCG() { mCG_Context = cgCreateContext(); }

//-----------------------------------------------------------------------

void cLowLevelGraphicsSDL::ExitCG() { cgDestroyContext(mCG_Context); }
#endif
