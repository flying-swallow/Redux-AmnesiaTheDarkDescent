...
TranslucentMeshPipelineDesc::TranslucentMeshPipelineDesc(
  ...
  // Strides match cVertexBuffer: position/tangent/color float4 (16B), normal
  // float3 (12B), texcoord float3 with only .xy consumed (12B). An optional
  // stream the renderable omits (absent from vertexPresentMask) gets its stride
  // zeroed so the bound single-vertex fallback feeds the same default.
