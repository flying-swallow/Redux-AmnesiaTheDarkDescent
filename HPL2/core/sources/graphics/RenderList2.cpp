...
namespace hpl {
    ...
    void cRenderList2::BeginAndReset(float frameTime, cFrustum* frustum) {
    ...
    void cRenderList2::AddObject(iRenderable* apObject) {
    ...
    void cRenderList2::PrintAllObjects() {
        Log("---------------------------------\n");
    ...
    bool cRenderList2::ArrayHasObjects(eRenderListType aType) {
        return m_sortedArrays[aType].empty() == false;
    }

    std::span<iRenderable*> cRenderList2::GetRenderableItems(eRenderListType aType) {
        if (m_sortedArrays[aType].empty()) {
            return std::span<iRenderable*>();
        }
        return std::span<iRenderable*>(m_sortedArrays[aType]);
    }
    std::span<iRenderable*> cRenderList2::GetSolidObjects() {
    ...
    std::span<iRenderable*> cRenderList2::GetTransObjects() {
    ...
    std::span<iLight*> cRenderList2::GetLights() {
        if (m_lights.empty()) {
