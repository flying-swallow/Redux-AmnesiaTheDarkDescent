...
cGuiGlobalShortcut *iWidget::AddShortcut(int alKeyModifiers, eKey aKey,
                                         eGuiMessage aMsg,
                                         bool abBypassVisibility,
                                         bool abBypassEnabled) {
  cGuiGlobalShortcut *pShortcut = mpSet->AddGlobalShortcut(
      alKeyModifiers, aKey, this, aMsg, abBypassVisibility, abBypassEnabled);
...
bool iWidget::ProcessMessage(eGuiMessage aMessage, const cGuiMessageData &aData,
                             bool abSkipVisCheck, bool abSkipEnabledCheck) {
  if (mbCallbacksDisabled || !abSkipVisCheck && IsVisible() == false ||
...
void iWidget::AddCallback(eGuiMessage aMessage,
                          std::function<bool(const cGuiMessageData &)> &&cb) {
  mvCallbackHandler[aMessage].push_back(cb);
}

void iWidget::AddCallback(eGuiMessage aMessage, void *apObject,
                          tGuiCallbackFunc apFunc) {
...
bool iWidget::HasFocus() { return mpSet->GetFocusedWidget() == this; }
...
void iWidget::SetGlobalPosition(const cVector3f &avPos) {
  if (mpParent)
    SetPosition(avPos - mpParent->GetGlobalPosition() -
                mpParent->GetChildrenOffset());
  else
    SetPosition(avPos);
}

const cVector3f &iWidget::GetLocalPosition() { return mvPosition; }
...
const cVector3f &iWidget::GetGlobalPosition() {
  if (mpParent) {
    if (mbPositionIsUpdated) {
      ...
      mvGlobalPosition =
          mpParent->GetGlobalPosition() + mpParent->GetChildrenOffset() -
          (mbAffectedByScroll ? mpParent->GetScrollAmount() : cVector3f(0)) +
          mvPosition;
    }
...
void iWidget::CenterGlobalPositionInSet() {
  ...
  SetGlobalPosition(cVector3f(vSetSize.x / 2.0f - mvSize.x / 2.0f,
                              vSetSize.y / 2.0f - mvSize.y / 2.0f, fGlobalZ));
}
...
cGuiGfxElement *iWidget::GetPointerGfx() { return mpPointerGfx; }
...
bool iWidget::OnGotFocus(const cGuiMessageData &aData) { return mbEnabled; }
...
bool iWidget::OnGotTabFocus(const cGuiMessageData &aData) { return mbEnabled; }

void iWidget::OnChildGotTabFocus(iWidget *apChild,
...
void iWidget::DrawBordersAndCorners(cGuiGfxElement *apBackground,
  ...
  if (apBackground) {
    if (abBordersOutside) {
      ...
      vSize = avSize - apCornerVec[2]->GetActiveSize() -
              apCornerVec[0]->GetActiveSize();
    }

    mpSet->DrawGfx(apBackground, vPos, vSize, cColor(1, 1));

    // mpSet->DrawGfx(apBackground,cVector3f(
    // apCornerVec[0]->GetActiveSize().x, 									apCornerVec[0]->GetActiveSize().y,0 ),
    //				avSize -
    //apCornerVec[2]->GetActiveSize()-apCornerVec[0]->GetActiveSize(),
    //				cColor(1,1));
  }
  ...
  if (abBordersOutside) {
    ...
    // Borders
    // Right
    mpSet->DrawGfx(apBorderVec[0], cVector3f(avSize.x, 0, 0),
                   cVector2f(apBorderVec[0]->GetImageSize().x, avSize.y));
    // Left
    mpSet->DrawGfx(apBorderVec[1],
                   cVector3f(-apCornerVec[0]->GetActiveSize().x, 0, 0),
                   cVector2f(apBorderVec[1]->GetImageSize().x, avSize.y));

    // Up
    mpSet->DrawGfx(apBorderVec[2],
                   cVector3f(0, -apCornerVec[0]->GetActiveSize().y, 0),
                   cVector2f(avSize.x, apBorderVec[2]->GetImageSize().y));

    // Down
    mpSet->DrawGfx(apBorderVec[3], cVector3f(0, avSize.y, 0),
                   cVector2f(avSize.x, apBorderVec[3]->GetImageSize().y));

    ///////////////////////
    // Corners
    // Left Up
    mpSet->DrawGfx(apCornerVec[0],
                   cVector3f(-apCornerVec[0]->GetActiveSize().x,
                             -apCornerVec[0]->GetActiveSize().y, 0));
    // Right Up
    mpSet->DrawGfx(apCornerVec[1],
                   cVector3f(avSize.x, -apCornerVec[1]->GetActiveSize().y, 0));

    // Right Down
    mpSet->DrawGfx(apCornerVec[2], cVector3f(avSize.x, avSize.y, 0));
    // Left Down
    mpSet->DrawGfx(apCornerVec[3],
                   cVector3f(-apCornerVec[3]->GetActiveSize().x, avSize.y, 0));
  } else {
    ///////////////////////
    // Borders
    // Right
    ...
    mpSet->DrawGfx(
        apCornerVec[1],
        cVector3f(avSize.x - apCornerVec[1]->GetActiveSize().x, 0, 0));

    // Right Down
    mpSet->DrawGfx(apCornerVec[2],
                   cVector3f(avSize.x - apCornerVec[2]->GetActiveSize().x,
                             avSize.y - apCornerVec[2]->GetActiveSize().y, 0));
    // Left Down
    mpSet->DrawGfx(
        apCornerVec[3],
        cVector3f(0, avSize.y - apCornerVec[3]->GetActiveSize().y, 0));
  }
...
void iWidget::DrawSkinText(const tWString &asText, eGuiSkinFont aFont,
  ...
  mpSet->DrawFont(asText, pFont->mpFont.Get(), avPosition, pFont->mvSize,
                  pFont->mColor * mColorMul, aAlign);
}
...
void iWidget::DrawDefaultTextHighlight(
    const tWString &asText, const cVector3f &avPosition,
    eFontAlign aAlign) //, int alFirstVisibleChar, int alHighlightStart, int
                       //alHighlightSize )
{
  if (mpDefaultFontType == NULL)
    return;
...
bool iWidget::ProcessCallbacks(eGuiMessage aMessage,
                               const cGuiMessageData &aData) {
  auto& lstCallbacks = mvCallbackLists[aMessage];

  if (lstCallbacks.empty())
    return false;

  bool bRet = false;
  auto it = lstCallbacks.begin();
  for (; it != lstCallbacks.end(); ++it) {
	...
	for(auto& it: mvCallbackHandler[aMessage]) {
		bRet |= it(aData);
	}
...
int iWidget::UIArrowToArrayPos(eUIArrow aDir) { return cMath::Log2ToInt(aDir); }
...
void iWidget::LoadGraphics() {
  if (mpSkin) {
    ...
    cVector3f vHighlightCol =
        mpSkin->GetAttribute(eGuiSkinAttribute_DefaultFontHighlightColor);
    mDefaultFontHighlightColor =
        cColor(vHighlightCol.x, vHighlightCol.y, vHighlightCol.z, 1);
  } else {
    mpDefaultFont = NULL;
  }
...
void iWidget::SetToolTip(const tWString &asToolTip) {
  ...
  if (GetSize() == 0) {
    float fWidth =
        mpDefaultFontType->GetLength(mvDefaultFontSize, msText.c_str());
