...
public:
  ...
  virtual bool Init(int alWidth, int alHeight, int alDisplay, int alBpp,
                    int abFullscreen, int alMultisampling,
  ...
  /**
   * Get the capabilities of the graphics. Th return value depends on
   * the capability
   * \return
   */
  ...
  virtual iGpuShader *CreateGpuShader(const tString &asName,
                                      eGpuShaderType aType) = 0;

  virtual iTexture *CreateTexture(const tString &asName, eTextureType aType,
                                  eTextureUsage aUsage) = 0;
  ...
  virtual void CopyFrameBufferToTexure(iTexture *apTex, const cVector2l &avPos,
                                       ...
                                       const cVector2l &avTexOffset = 0) = 0;

  /**
   * Copies a portion of the screen to the full size of a bitmap.
   * \param &avScreenPos The position on the screen
   * \param &avScreenSize The size of the portion of the screen. -1 =
   * use full screen size.
   * \return a bitmap that contains the data. The bitmap is created by
   * the method and must be deleted by the user.
   */
  virtual cBitmap *
  CopyFrameBufferToBitmap(const cVector2l &avScreenPos = 0,
                          const cVector2l &avScreenSize = -1) = 0;

  /**
   * All further drawing operations are rendered to this texture.
   * \param pTex Texture to render to. NULL = screen (frame buffer)
   */
  virtual void SetCurrentFrameBuffer(iFrameBuffer *apFrameBuffer,
                                     const cVector2l &avPos = 0,
                                     const cVector2l &avSize = -1) = 0;
  ...
  /**
   * Permanently sets a different setting of draw targets to current
   * frame buffer.
   * \param *apTargets a vector of integers, each corresponding to a
   * draw buffer in the current
   * \param alNumOfTargets
   */
  virtual void SetFrameBufferDrawTargets(int *apTargets,
                                         int alNumOfTargets) = 0;
  ...
  virtual void SetStencil(eStencilFunc aFunc, int alRef, unsigned int aMask,
                          eStencilOp aFailOp, eStencilOp aZFailOp,
                          eStencilOp aZPassOp) = 0;
  virtual void
  SetStencilTwoSide(eStencilFunc aFrontFunc, eStencilFunc aBackFunc, int alRef,
                    unsigned int aMask, eStencilOp aFrontFailOp,
                    eStencilOp aFrontZFailOp, eStencilOp aFrontZPassOp,
                    eStencilOp aBackFailOp, eStencilOp aBackZFailOp,
                    eStencilOp aBackZPassOp) = 0;

  virtual void SetScissorActive(bool abX) = 0;
  virtual void SetScissorRect(const cVector2l &avPos,
                              const cVector2l &avSize) = 0;
  ...
  virtual void SetBlendFuncSeparate(eBlendFunc aSrcFactorColor,
                                    eBlendFunc aDestFactorColor,
                                    eBlendFunc aSrcFactorAlpha,
                                    eBlendFunc aDestFactorAlpha) = 0;
  ...
  virtual void SetOrthoProjection(const cVector2f &avSize, float afMin,
                                  float afMax) = 0;
  virtual void SetOrthoProjection(const cVector3f &avMin,
                                  const cVector3f &avMax) = 0;
  ...
  virtual void DrawQuad(const tVertexVec &avVtx, const float afZ,
                        const cColor &aCol) = 0;
  virtual void DrawQuadMultiTex(const tVertexVec &avVtx,
                                const tVector3fVec &avExtraUvs) = 0;

  virtual void DrawLine(const cVector3f &avBegin, const cVector3f &avEnd,
                        cColor aCol) = 0;
  virtual void DrawLine(const cVector3f &avBegin, const cColor &aBeginCol,
                        const cVector3f &avEnd, const cColor &aEndCol) = 0;
  virtual void DrawBoxMinMax(const cVector3f &avMin, const cVector3f &avMax,
                             cColor aCol) = 0;
  virtual void DrawSphere(const cVector3f &avPos, float afRadius,
                          cColor aCol) = 0;
  virtual void DrawSphere(const cVector3f &avPos, float afRadius, cColor aColX,
                          cColor aColY, cColor aColZ) = 0;

  virtual void DrawLineQuad(const cRect2f &aRect, float afZ, cColor aCol) = 0;
  virtual void DrawLineQuad(const cVector3f &avPos, const cVector2f &avSize,
                            cColor aCol) = 0;
  ...
  virtual void AddVertexToBatch(const cVertex *apVtx,
                                const cVector3f *avTransform) = 0;
  virtual void AddVertexToBatch(const cVertex *apVtx, const cMatrixf *aMtx) = 0;

  virtual void AddVertexToBatch_Size2D(const cVertex *apVtx,
                                       const cVector3f *avTransform,
                                       const cColor *apCol, const float &mfW,
                                       const float &mfH) = 0;

  virtual void AddVertexToBatch_Raw(const cVector3f &avPos,
                                    const cColor &aColor,
                                    const cVector3f &avTex) = 0;

  virtual void AddIndexToBatch(int alIndex) = 0;

  virtual void AddTexCoordToBatch(unsigned int alUnit,
                                  const cVector3f *apCoord) = 0;
  virtual void SetBatchTextureUnitActive(unsigned int alUnit,
                                         bool abActive) = 0;

  virtual void FlushTriBatch(tVtxBatchFlag aTypeFlags,
                             bool abAutoClear = true) = 0;
  virtual void FlushQuadBatch(tVtxBatchFlag aTypeFlags,
                              bool abAutoClear = true) = 0;
