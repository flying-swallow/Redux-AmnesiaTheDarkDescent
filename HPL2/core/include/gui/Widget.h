...
public:
  ...
  bool ProcessMessage(eGuiMessage aMessage, const cGuiMessageData &aData,
                      bool abSkipVisCheck = false,
                      bool abSkipEnabledCheck = false);

	void AddCallback(eGuiMessage aMessage,std::function<bool(const cGuiMessageData&)>&& cb);
  ...
  AddShortcut(int alKeyModifiers, eKey aKey,
...
protected:
  ...
  virtual void OnChildGotTabFocus(iWidget *apChild,
  ...
  void DrawSkinText(const tWString &asText, eGuiSkinFont aFont,
                    const cVector3f &avPosition,
                    eFontAlign aAlign = eFontAlign_Left);

  void DrawDefaultText(const tWString &asText, const cVector3f &avPosition,
                       eFontAlign aAlign);

  void DrawDefaultText(const tWString &asText, const cVector3f &avPosition,
                       eFontAlign aAlign, const cColor &aCol);

  void DrawDefaultTextHighlight(
...
private:
  ...
  std::array<std::vector<cWidgetCallback>, eGuiMessage_LastEnum> mvCallbackLists;
	std::array<std::vector<std::function<bool(const cGuiMessageData&)>>, eGuiMessage_LastEnum> mvCallbackHandler;
